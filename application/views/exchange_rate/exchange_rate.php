<!-- main content -->
<section class="content home">
    <div class="container-fluid">
        <div class="block-header">
            <h2>อัตราแลกเปลี่ยน</h2>
            <ul class="breadcrumb">         
                <li class="breadcrumb-item active"><i class="material-icons">attach_money</i> อัตราแลกเปลี่ยน</li>
            </ul>
        </div>



        <div class="row clearfix">
            <div class="col-lg-12 col-md-12 col-sm-12">
                <div class="card">
                    <div class="header">
                        <h2>อัตราแลกเปลี่ยน</h2>                     
                    </div>
                    <div class="body">
                        <div class="row">                           
                            <div class="col-sm-12"> 
                                <p class="indent"> อัตราแลกเปลี่ยนด้านล่างนี้ เป็นอัตราแลกเปลี่ยนของทางบริษัท เพย์ โซลูชั่น จำกัด ที่ใช้อ้างอิงสำหรับร้านค้าที่ต้องการจำหน่ายสินค้าเป็นสกุลเงินตราต่างประเทศ ทั้งนี้ ทางบริษัทขอสงวนสิทธิ์ในการเปลี่ยนแปลงอัตราแลกเปลี่ยนโดยไม่ต้องแจ้งให้ทราบล่วงหน้าได้  </p>
                            </div>  
                        </div>
                    </div>

                </div>
            </div>
        </div>

        <div class="row clearfix">
            <div class="col-lg-12 col-md-12 col-sm-12">
                <div class="card">
                    <div class="body">
                        <div class="row clearfix">
                            <div class="col-md-2"> 
                                <div class="form-group">
                                    <b>ระหว่างวันที่</b>
                                </div>
                            </div>
                            <div class="col-md-3">
                                <div class="form-group">
                                    <div class="input-group"> <span class="input-group-addon"> <i class="material-icons">date_range</i> </span>
                                        <div class="form-line">
                                            <input type="text" class="datepicker form-control datetime" placeholder="Please choose Date...">
                                        </div>
                                    </div>
                                </div>
                            </div>
                            <div class="col-sm-1"> 
                                <div class="form-group text-center">
                                    <b>ถึง</b>
                                </div>
                            </div>
                            <div class="col-md-3">
                                <div class="form-group">
                                    <div class="input-group"> <span class="input-group-addon"> <i class="material-icons">date_range</i> </span>
                                        <div class="form-line">
                                            <input type="text" class="datepicker form-control datetime" placeholder="Please choose Date...">
                                        </div>
                                    </div>
                                </div>
                            </div>

                            <div class="body">
                                <div class="button-demo">
                                    <button type="button" class="btn btn-raised btn-primary waves-effect">SUMBIT</button>
                                </div>
                            </div>
                        </div>
                        </br>
                        <div class="body table-responsive">
                            <table class="table table-bordered" width="100%">
                                <thead>
                                    <tr>
                                        <th>USD</th>
                                        <th>JPY</th>
                                        <th>SGD</th>
                                        <th>HKD</th>
                                        <th>EUR</th>
                                        <th>GBP</th>
                                        <th>AUD</th>
                                        <th>CHF</th>
                                        <th>Input Date Time</th>
                                    </tr>
                                </thead>

                                <tr>
                                    <td>31.36</td>
                                    <td>27.8200</td>
                                    <td>22.59</td>
                                    <td>3.65</td>
                                    <td>36.40</td>
                                    <td>40.89</td>
                                    <td>23.51</td>
                                    <td>31.90</td>
                                    <td>14-11-2017 10:44:50</td>
                                </tr><tr>
                                    <td>31.39</td>
                                    <td>27.9300</td>
                                    <td>22.69</td>
                                    <td>3.66</td>
                                    <td>36.41</td>
                                    <td>41.19</td>
                                    <td>23.51</td>
                                    <td>31.86</td>
                                    <td>13-11-2017 11:25:57</td>
                                </tr><tr>
                                    <td>31.40</td>
                                    <td>27.9300</td>
                                    <td>22.69</td>
                                    <td>3.66</td>
                                    <td>36.30</td>
                                    <td>41.01</td>
                                    <td>23.51</td>
                                    <td>31.86</td>
                                    <td>10-11-2017 12:06:14</td>
                                </tr><tr>
                                    <td>31.41</td>
                                    <td>27.7600</td>
                                    <td>23.34</td>
                                    <td>3.66</td>
                                    <td>36.30</td>
                                    <td>41.27</td>
                                    <td>23.51</td>
                                    <td>31.81</td>
                                    <td>09-11-2017 10:05:29</td>
                                </tr><tr>
                                    <td>31.54</td>
                                    <td>27.8600</td>
                                    <td>23.24</td>
                                    <td>3.66</td>
                                    <td>36.32</td>
                                    <td>41.25</td>
                                    <td>24.27</td>
                                    <td>31.88</td>
                                    <td>08-11-2017 16:40:07</td>
                                </tr><tr>
                                    <td>31.94</td>
                                    <td>28.6880</td>
                                    <td>23.19</td>
                                    <td>4.22</td>
                                    <td>36.89</td>
                                    <td>41.53</td>
                                    <td>24.94</td>
                                    <td>32.77</td>
                                    <td>07-11-2017 10:01:10</td>
                                </tr><tr>
                                    <td>33.02</td>
                                    <td>28.6350</td>
                                    <td>24.00</td>
                                    <td>4.23</td>
                                    <td>38.10</td>
                                    <td>42.90</td>
                                    <td>24.84</td>
                                    <td>32.75</td>
                                    <td>06-11-2017 09:57:22</td>
                                </tr><tr>
                                    <td>32.93</td>
                                    <td>28.6630</td>
                                    <td>24.04</td>
                                    <td>4.21</td>
                                    <td>38.17</td>
                                    <td>42.74</td>
                                    <td>24.94</td>
                                    <td>32.74</td>
                                    <td>03-11-2017 09:26:44</td>
                                </tr><tr>
                                    <td>33.01</td>
                                    <td>28.7510</td>
                                    <td>24.03</td>
                                    <td>4.22</td>
                                    <td>38.18</td>
                                    <td>43.56</td>
                                    <td>24.90</td>
                                    <td>32.81</td>
                                    <td>01-11-2017 10:12:38</td>
                                </tr>
                            </table>
                        </div>
                    </div>
                </div>
         
        
